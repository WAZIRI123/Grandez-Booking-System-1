...
<div class="h-full bg-gray-200 p-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-10">
            ...
            <div class="flex items-center justify-between mt-6">
                ...
                <div class="flex flex-col">
                    <div class="flex items-end">
                        <span class="text-2xl 2xl:text-4xl font-bold">{{ $transferreservation->where('status','confirmed')->count() }}</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="px-6 py-6 bg-white rounded-lg shadow-xl">
            ...
            <div class="flex items-center justify-between mt-6">
                ...
                <div class="flex flex-col">
                    <div class="flex items-end">
                        <span class="text-2xl 2xl:text-4xl font-bold">{{ $transferreservation->where('status','rejected')->count() }}</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="px-6 py-6 bg-white rounded-lg shadow-xl">
            ...
            <div class="flex items-center justify-between mt-6">
                ...
                <div class="flex flex-col">
                    <div class="flex items-end">
                        <span class="text-2xl 2xl:text-4xl font-bold">{{ $transferreservation->where('status','waiting')->count() }}</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="px-6 py-6 bg-white rounded-lg shadow-xl">
            ...
            <div class="flex items-center justify-between mt-6">
                ...
                <div class="flex flex-col">
                    <div class="flex items-end">
                        <span class="text-2xl 2xl:text-4xl font-bold">{{ $transferreservation->count() }}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-lg px-8 py-6 my-16 overflow-x-scroll custom-scrollbar">
        ...
        <table class="w-full my-8 whitespace-nowrap">
            ...
            <tbody class="text-sm">
                @forelse ($transferreservations as $transferreservation)
                <tr class="bg-gray-100 hover:bg-primary hover:bg-opacity-20 transition duration-200">
                    <td class="py-3 pl-2">
                        <input type="checkbox" class="rounded focus:ring-0 checked:bg-red-500 ml-2">
                    </td>
                    <td class="py-3 pl-2">
                        {{$transferreservation->date}}
                    </td>
                    <td class="py-3 pl-2 capitalize">
                        {{$transferreservation->transfers->name}}
                    </td>
                    <td class="py-3 pl-2">
                        <span class="bg-green-500 px-1.5 py-0.5 rounded-lg text-gray-100">{{$transferreservation->date}}</span>
                    </td>
                    <td class="py-3 pl-2">
                    {{$transferreservation->status}}
                    </td>
                    <td class="py-3 pl-2">
                            <!-- start::Rounded Select -->
                            <div class="flex flex-col">
                                <select name="{{$transferreservation->id}}" wire:model="status" wire:change="changeStatus({{$transferreservation->id}})" id="{{$transferreservation->id}}" class="mt-2 py-1 border-gray-300 focus:outline-none focus:ring-0 focus:border-gray-300 rounded-lg">
                    ...
                    <td class="py-3 pl-2">
                        <a href="#" class="bg-primary hover:bg-opacity-90 px-2 py-1 mr-2 text-gray-100 rounded-lg">${{$transferreservation->transfers->price}}</a>
                    </td>
                </tr>
                @empty
                <tr>
