...
<div class="h-full bg-gray-200 p-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-10">
            ...
            <div class="flex items-center justify-between mt-6">
                ...
                <div class="flex flex-col">
                    <div class="flex items-end">
                        <span class="text-2xl 2xl:text-4xl font-bold">{{ $packagereservation->where('status','confirmed')->count() }}</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="px-6 py-6 bg-white rounded-lg shadow-xl">
            ...
            <div class="flex items-center justify-between mt-6">
                ...
                <div class="flex flex-col">
                    <div class="flex items-end">
                        <span class="text-2xl 2xl:text-4xl font-bold">{{ $packagereservation->where('status','rejected')->count() }}</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="px-6 py-6 bg-white rounded-lg shadow-xl">
            ...
            <div class="flex items-center justify-between mt-6">
                ...
                <div class="flex flex-col">
                    <div class="flex items-end">
                        <span class="text-2xl 2xl:text-4xl font-bold">{{ $packagereservation->where('status','waiting')->count() }}</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="px-6 py-6 bg-white rounded-lg shadow-xl">
            ...
            <div class="flex items-center justify-between mt-6">
                ...
                <div class="flex flex-col">
                    <div class="flex items-end">
                        <span class="text-2xl 2xl:text-4xl font-bold">{{ $packagereservation->count() }}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-lg px-8 py-6 my-16 overflow-x-scroll custom-scrollbar">
        ...
        <table class="w-full my-8 whitespace-nowrap">
            ...
            <tbody class="text-sm">
                @forelse ($packagereservations as $packagereservation)
                <tr class="bg-gray-100 hover:bg-primary hover:bg-opacity-20 transition duration-200">
                    <td class="py-3 pl-2">
                        <input type="checkbox" class="rounded focus:ring-0 checked:bg-red-500 ml-2">
                    </td>
                    <td class="py-3 pl-2">
                        {{$packagereservation->start_date}}
                    </td>
                    <td class="py-3 pl-2 capitalize">
                        {{$packagereservation->packages->name}}
                    </td>
                    <td class="py-3 pl-2">
                        {{$packagereservation->no_of_visitors}}
                    </td>
                    <td class="py-3 pl-2">
                        <span class="bg-green-500 px-1.5 py-0.5 rounded-lg text-gray-100">{{$packagereservation->end_date}}</span>
                    </td>
                    <td class="py-3 pl-2">
                    {{$packagereservation->status}}
                    </td>
                    <td class="py-3 pl-2">
                            <!-- start::Rounded Select -->
                            <div class="flex flex-col">
                                <select name="{{$packagereservation->id}}" wire:model="status" wire:change="changeStatus({{$packagereservation->id}})" id="{{$packagereservation->id}}" class="mt-2 py-1 border-gray-300 focus:outline-none focus:ring-0 focus:border-gray-300 rounded-lg">
                    ...
                    <td class="py-3 pl-2">
                        <a href="#" class="bg-primary hover:bg-opacity-90 px-2 py-1 mr-2 text-gray-100 rounded-lg">${{$packagereservation->total_price}}</a>
                    </td>
                </tr>
                @empty
                <tr>
